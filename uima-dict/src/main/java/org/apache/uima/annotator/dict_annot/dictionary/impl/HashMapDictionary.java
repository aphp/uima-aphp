...
/**
 * HashMap dictionary implementation. Implements the Dictionary interface using a simple HashMap for
 * the lookup. Each word added to the dictionary is added to the main dictionary HashMap with some
 * additional meta data information. In case of multi-words the multi word parts are added as tree
 * to the dictionary.
 */
public class HashMapDictionary implements Dictionary {
  ...
  private boolean accentNormalization = true;

  private HashSet<String> stopWords = new HashSet<>(Arrays.asList("de"));

  private SnowballStemmer a;
   ...
   */
  public HashMapDictionary(boolean caseNormalization, boolean accentNormalization,
  ...
  /**
   * check if the given multi-word is available in the dictionary. If it is available the
   * DictionaryEntry is returned. If it is not available, null is returned
   * 
   * @param multiWord multi-word used for the lookup
   * 
   * @return DictionaryEntry for the given multi-word, or null if the entry is not in the dictionary
   */
   ...
   * (non-Javadoc)
   * 
   * @see org.apache.uima.annotator.dict_annot.Dictionary#matchEntry(int, java.lang.String[])
   */
   ...
   * 
   * @param word word that should be added to the dictionary
   * 
  ...
  public int addWord(String word) {
    ...
    this.dictionary.put(normalizeString(word), new DictionaryEntry(true, metaData));
    return this.idCounter;
  }
   ...
   * 
   * @param multiWord multi-word that should be added to the dictionary
   * 
   * @return ID that was generated for this entry
   */
  public int addMultiWord(String[] multiWord) {
    ...
    for (int i = 0; i < multiWord.length; i++) {
      if (this.stopWords.contains(multiWord[i])) {
        continue;
      }
      ...
      if (currentMap.containsKey(normalizeString(multiWord[i]))) {
        ...
        currentMap = (currentMap.get(normalizeString(multiWord[i]))).getSubBranch();
  ...
  /**
   * print the dictionary content either to an Writer object or if not output is specified to the
   * command line.
   * 
   * @param out Writer object to write the content to
   * 
   * @throws IOException
   */
  ...
  /**
   * search the first longest matches for the given input data. This method is called recursive to
   * detect multi word matches. All valid matches that are found are stored so that they can be
   * retrieved later.
   * 
   * @param pos start position in the token array
   * @param tokens token input array
   * @param dict dictionary to use
   * @param match match result object where all match data is stored
   * @param offset offset in relation to the start position of the current token that is discovered
   */
  private void checkMatches(int pos, AnnotationFS[] annotFSs, FeaturePathInfo featPathInfo,
      HashMap<String, DictionaryEntry> dict, DictionaryMatchImpl match, int offset) {
    ...
    if (value != null) {
      ...
      if (dict.containsKey(normalizedValue)) {
        ...
        if (annotFSs.length > pos + offset) {
          // ... investigate them
          checkMatches(pos, annotFSs, featPathInfo, currentEntry.getSubBranch(), match, offset);
        }
      }
    }
  }
   ...
   * 
   * @param map current dictionary branch
   * 
   * @param previousTokens previous tokens for the current branch
   */
  private void printBranch(HashMap<String, DictionaryEntry> map, ArrayList<String> previousTokens,
  ...
  /**
   * Converts the array list of string to a concatenated String separated by a whitespace character.
   * 
   * @param stringList array list of string
   * 
  ...
  /**
   * Normalize the input string to lower case and remove all spaces around if the dictionary is
   * configured to do a case normalization
   * 
  ...
  private String normalizeString(String input) {
    ...
    if (!"".equals(this.snowballStemmer)) {
      ALGORITHM algo = ALGORITHM.FRENCH;
      if (a == null) {
        switch (snowballStemmer.toLowerCase()) {
          case "french":
            algo = ALGORITHM.FRENCH;
            break;
          case "english":
        ...
        a = new SnowballStemmer(algo);
      }
      input = a.stem(input).toString();
    }

    if (this.accentNormalization) {
