...
/**
 * The DicrionaryCreator creates an dictionary XML file based on the given input parameters. The
 * input file must contain one dictionary entry for each line. The line is tokenized into multi
 * words either with the specified tokenizer PEAR file or with the separator character.
 * 
 * The dictionary output XML file (UTF-8) must be updated after the creation with some metadata
 * information.
 */
public class DictionaryCreatorStanford {
  ...
  private static String ACCENT_NORMALIZATION = "-accentnormalization";

  private static String MULTI_WORD = "-multiword";

  private static String SNOWBALL_STEMMER = "-snowballstemmer";

  private static HashMap<Character, String> entities;

  private static String DICTIONARY_TYPE = "-dictionarytype";
  ...
  /**
   * creates the CommandLine parser used to parse the DictionaryCreator command line.
   * 
  ...
  private static final CommandLineParser createCmdLineParser() {
    ...
    parser.addParameter(STOP_WORDS, true);
    parser.addParameter(ACCENT_NORMALIZATION, true);
    parser.addParameter(CASE_SENSITIVE, true);
    parser.addParameter(MULTI_WORD, true);
    parser.addParameter(DICTIONARY_TYPE, true);
    parser.addParameter(SNOWBALL_STEMMER, true);
    return parser;
  }
  ...
  private static final void printUsage() {
    System.out.println(
        "Usage: java org.apache.uima.annotator.dict_annot.dictionary.impl.DictionaryCreator -input <InputFile> -encoding <InputFileEncoding> -output <OutputFile> -stopwords <StopWords> -accentnormalization <TrueOrFalse> -casesensitive <TrueOrFalse> -multiword <TrueOrFalse> -dictionarytype <fr.aphp.wind.uima.type.dictx> [-separator <separatorChar>] ");
   ...
   * 
   * @param clp current CommandLineParser
   * 
   * @return returns false in case of command line syntax errors, otherwise true
   */
  private static final boolean checkCmdLineSyntax(CommandLineParser clp) {
    ...
    if (!clp.isInArgsList(CASE_SENSITIVE)) {
      System.err.println("Casesensitive parameter -casesensitive is missing");
      error = true;
    }
    if (!clp.isInArgsList(ACCENT_NORMALIZATION)) {
      System.err.println("Accentnormalization parameter -accentnormalization is missing");
      error = true;
    }
    if (!clp.isInArgsList(DICTIONARY_TYPE)) {
      System.err.println("Dictionarytype parameter -dictionarytype is missing");
      error = true;
    }
    if (!clp.isInArgsList(STOP_WORDS)) {
      System.err.println("Stopŵord parameter -stopword is missing");
      error = true;
    }
    if (!clp.isInArgsList(SNOWBALL_STEMMER)) {
      System.err.println("Snowballstemmer parameter -snowballstemmer is missing");
      error = true;
    }
    ...
    if (clp.isInArgsList(TOKENIZER_PARAM)) {
      if (!clp.isInArgsList(TOKEN_TYPE_PARAM)) {
        System.err.println("If a tokenizer is used, the -tokenType parameter must be specified");
        error = true;
      }
    }
  ...
  public static void main(String[] args) {
    ...
    String caseSensitive = clp.getParamArgument(CASE_SENSITIVE);
    String accentNormalization = clp.getParamArgument(ACCENT_NORMALIZATION);
    String stopWords = clp.getParamArgument(STOP_WORDS);
    String multiWord = clp.getParamArgument(MULTI_WORD);
    String dictionaryType = clp.getParamArgument(DICTIONARY_TYPE);
    String snowballStemmer = clp.getParamArgument(SNOWBALL_STEMMER);

    // create dictionary
    try {
      DictionaryCreatorStanford.createDictionary(inputFile, encoding, outputFile, language,
          tokenizerFile, tokenTypeStr, separatorChar, caseSensitive, accentNormalization, stopWords,
          multiWord, dictionaryType, snowballStemmer);
  ...
  public static boolean createDictionary(String inputFile, String encoding, String outputFile,
      String language, String tokenizerFile, String tokenTypeStr, String separatorChar,
      String caseSensitive, String accentNormalization, String stopWords, String multiWord,
      String dictionaryType, String snowballStemmer) throws Exception {

    String outputSeparatorChar = OUTPUT_SEPARATOR_CHAR;
    String globalConfig = String.format(
        "caseSensitive='%s';multiWord='%s';multiWordSeparator='%s';accentNormalization='%s';snowballStemmer='%s';stopWords='%s'",
        caseSensitive, multiWord, " ", accentNormalization, snowballStemmer,stopWords);
    ...
    if (tokenizerFile != null) {
      ...
      try {
        ...
        PackageBrowser instPear = PackageInstaller.installPackage(tempDir, pearFile, true);
    ...
    BufferedReader reader =
        new BufferedReader(new InputStreamReader(new FileInputStream(inputFile), encoding));
    ...
    writer.write("<typeName>" + dictionaryType + "</typeName>\n");
    writer.write("</typeDescription>\n");
  ...
  private static String replaceXMLEntities(String text) {
    ...
    for (int i = 0; i < text.length(); i++) {
      ...
      if (DictionaryCreatorStanford.entities.containsKey(character)) {
        buffer.append(DictionaryCreatorStanford.entities.get(character));
      } else {
